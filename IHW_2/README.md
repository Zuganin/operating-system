...
`common.h`
Содержит:
- Ключи `SHM_KEY`, `SEM_KEY`
- Размеры очередей и количество семафоров
- Структура shm_t для организации разделяемой памяти
- Количество читателей (M)
...
- Разделяемую память
- Семафоры
- Массив книг (все книги доступны)
- Очередь уведомлений и флаг done = 0

---

`reader.c`
Каждый читатель:
- Получает случайный список книг
- Ожидает доступности книги (семафоры)
- "Читает" книгу (имитация через sleep)
- Возвращает книгу (обновляет shm, уведомляет библиотекаря)
- В конце отправляет специальное сообщение -1 в очередь для завершения библиотекаря

---

`librarian.c`
Бесконечный цикл:
- Ожидает уведомлений от читателей
- Извлекает книгу из очереди
- Уведомляет читателя, что книга снова доступна
- Завершается при получении сигнала (-1) и флага done = 1

---

`run.sh`
Автоматизирует:
- Очистку старых IPC объектов
- Запуск инициализации ./init
- Запуск библиотекаря в фоне ./librarian &
- Запуск N читателей ./reader $i
- Ожидание завершения всех читателей
- Отправку сигнала библиотекарю
- Очистку ресурсов

---

## Изменения и особенности реализации
Структура разделяемой памяти памяти и семафоров в стандарте UNIX SYSTEM V.
```
typedef struct {
    int head, tail;
    int queue[MAXQ];
...
```
## Очистка ресурксов
`ipcrm -M 0x1234` — удаляет сегмент разделяемой памяти с ключом 0x1234 (соответствует SHM_KEY);
`ipcrm -S 0x5678` — удаляет набор семафоров с ключом 0x5678 (соответствует SEM_KEY).
В каждом процессе присутствует:
```
shmdt(shm);
```
...
[Запуск] Библиотекарь PID=50772
[Запуск] Читатель 1
[Запуск] Читатель 2
[Запуск] Читатель 3
[Читатель 1] хочет прочитать 1 книг(-и)
[Читатель 1] взял книгу 2
[Читатель 1] читает книгу 2 1 дней
[Читатель 2] хочет прочитать 3 книг(-и)
[Читатель 2] взял книгу 1
[Читатель 2] читает книгу 1 2 дней
[Читатель 3] хочет прочитать 2 книг(-и)
[Читатель 3] взял книгу 0
[Читатель 3] читает книгу 0 2 дней
[Читатель 1] вернул книгу 2
[Библиотекарь] уведомляю читателя о книге 2
[Читатель 2] вернул книгу 1
[Библиотекарь] уведомляю читателя о книге 1
[Библиотекарь] уведомляю читателя о книге 0
[Читатель 3] вернул книгу 0
[Читатель  1] завершил чтение
[Читатель 2] взял книгу 0
[Читатель 2] читает книгу 0 2 дней
[Читатель 3] взял книгу 2
[Читатель 3] читает книгу 2 1 дней
[Читатель 3] вернул книгу 2
[Библиотекарь] уведомляю читателя о книге 2
[Читатель 2] вернул книгу 0
[Библиотекарь] уведомляю читателя о книге 0
[Читатель  3] завершил чтение
[Библиотекарь] Все читатели завершили работу. Завершаюсь.
[Читатель 2] взял книгу 2
[Читатель 2] читает книгу 2 2 дней
[Читатель 2] вернул книгу 2
[Читатель  2] завершил чтение
[Библиотекарь] Завершаю свою работу, все читатели завершили.
[Завершение] Отправлен сигнал завершения библиотекарю
[Очистка] Shared memory и семафоры удалены
```