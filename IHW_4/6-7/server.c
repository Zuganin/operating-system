...
void send_to_monitor(int sockfd, const char *event_type, const char *message);
...
void sigint_handler(int sig) {
    ...
    if (sockfd_global != -1) {
        ...
        send_to_monitor(sockfd_global, MONITOR_SHUTDOWN, "Сервер завершает работу по сигналу SIGINT");
...
void register_monitor(struct sockaddr_in *monitor_addr, socklen_t monitor_len, int sockfd) {
    ...
    if (sendto(sockfd, MONITOR_ACK, strlen(MONITOR_ACK), 0,
        ...
        printf("Монитор зарегистрирован: %s:%d\n",
    ...
    if (monitor.registered) {
        ...
        if (full_message) {
            ...
            if (sendto(sockfd, full_message, strlen(full_message), 0,
                       (struct sockaddr*)&monitor.addr, monitor.addr_len) < 0) {
                // Если не удалось отправить, помечаем монитор как отключенный
...
// Функция для отправки исходного текста на монитор  
void send_original_text_to_monitor(int sockfd) {
    if (portfolio.original_text) {
        ...
        if (monitor_message) {
            ...
            send_to_monitor(sockfd, SYSTEM_EVENT, monitor_message);
...
void update_client_info(struct sockaddr_in *client_addr, socklen_t client_len, int sockfd) {
    ...
    if (num_clients_connected < max_clients) {
        ...
        snprintf(message, BUFFER_SIZE, "Клиент подключен %s:%d (всего клиентов: %d)",
                inet_ntoa(client_addr->sin_addr), ntohs(client_addr->sin_port), num_clients_connected);
        send_to_monitor(sockfd, SYSTEM_EVENT, message);
    }
...
void notify_all_clients_finished(int sockfd) {
    ...
    send_to_monitor(sockfd, SERVER_EVENT, "Уведомляем всех клиентов о завершении работы");
    ...
    for (int i = 0; i < num_clients_connected; i++) {
        if (sendto(sockfd, NO_MORE_TASKS, strlen(NO_MORE_TASKS), 0,
            ...
            snprintf(message, BUFFER_SIZE, "Уведомление о завершении отправлено клиенту %s:%d",
                    inet_ntoa(clients[i].addr.sin_addr), ntohs(clients[i].addr.sin_port));
            send_to_monitor(sockfd, SERVER_EVENT, message);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    while (server_running) {
        ...
        if (strcmp(buffer, MONITOR_REGISTER) == 0) {
            ...
            send_original_text_to_monitor(sockfd);
            continue;
        }
        ...
        if (strcmp(buffer, REQUEST_TASK) == 0) {
            ...
            if (task == NULL) {
                ...
                send_to_monitor(sockfd, SERVER_EVENT, "Задачи закончились - отправляем сигнал завершения клиенту");
            } else {
                ...
                if (token) {
                    ...
                    if (token) {
                        ...
                        if (all_results_received()) {
                            ...
                            send_to_monitor(sockfd, SYSTEM_EVENT, "Все фрагменты получены - формируется итоговый результат");
                            notify_all_clients_finished(sockfd);
                            printf("Все фрагменты обработаны. Сервер завершает работу.\n");
                            send_to_monitor(sockfd, SYSTEM_EVENT, "Сервер завершает работу");
                            break;
                        }
                    }
                }
    ...
    // Отправляем сигнал завершения монитору
    send_to_monitor(sockfd, MONITOR_SHUTDOWN, "Сервер завершает работу");
