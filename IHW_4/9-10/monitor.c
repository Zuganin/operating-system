        ...
        if (select_result == 0) {
            ...
            if (received > 0) {
                ...
                if (strncmp(buffer, MONITOR_SHUTDOWN, strlen(MONITOR_SHUTDOWN)) == 0) {
                    // Обработка завершения: поддержка как "MONITOR_SHUTDOWN" так и "MONITOR_SHUTDOWN:..."
                    char *event_data = buffer + strlen(MONITOR_SHUTDOWN);
                    if (*event_data == ':' || *event_data == ' ')
                        event_data++;
                    get_timestamp(timestamp, sizeof(timestamp));
                    printf("[%s] ЗАВЕРШЕНИЕ: %s\n", timestamp, *event_data ? event_data : "Сервер завершил работу. Монитор автоматически завершается.");
