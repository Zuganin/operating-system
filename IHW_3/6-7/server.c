...
int main(int argc, char *argv[])
{
    ...
    while(winnieFound == 0) {
        ...
        /* Клиент подключен! */
        printf("[Сервер] ПОДКЛЮЧЕНИЕ: Стая пчел подключилась с %s:%d\n", 
               inet_ntoa(ClntAddr.sin_addr), ntohs(ClntAddr.sin_port));
        
        /* Отправляем уведомление о подключении клиента мониторам */
        char clientConnectMsg[BUFSIZE];
        sprintf(clientConnectMsg, "Стая пчел подключилась с %s:%d", 
                inet_ntoa(ClntAddr.sin_addr), ntohs(ClntAddr.sin_port));
        SendToMonitors(clientConnectMsg);
